...
describe('Gmail copy safeguards',()=>{
 ...
 it('handles both business uploads without dropping hosted images',()=>{const result=prepareGmailExport({...defaultProfile,mode:'business',photoUrl:'data:image/png;base64,YQ==',logoUrl:'https://example.com/logo.png'},{omitUploads:true});expect(result.ready).toBe(true);expect(result.html).not.toContain('data:image');expect(result.html).toContain('https://example.com/logo.png');});
 it('blocks excessive link markup even when no uploads exist',()=>{const result=prepareGmailExport({...defaultProfile,links:Array.from({length:100},(_,i)=>({enabled:true,label:'LinkedIn',url:`https://example.com/${i}`}))});expect(result.ready).toBe(false);expect(result.hasUploads).toBe(false);});

...
export function personalSignature(p, { preview, accent, background, text, escapeHtml: esc, safeImage }) {
  ...
  const horizontal = `<tr>${rail?`<td width="${railWidth}" style="width:${railWidth}px;padding:0;vertical-align:middle;">${rail}</td>${spacer(29)}`:''}${frame?`<td width="${photoSize+40}" style="width:${photoSize+40}px;padding:0;vertical-align:middle;">${frame}</td>${spacer(42)}`:''}<td style="padding:0;vertical-align:middle;">${details}</td></tr>`;

...
export function personalSignature(p, { preview, accent, background, text, escapeHtml: esc, safeImage }) {
  ...
  const rail = links.length ? `<table ${table} width="28" style="width:28px;">${links.map((l,i) => `<tr><td align="center" style="padding:0;text-align:center;"><a href="${esc(l.url)}" title="${esc(l.label)}" style="color:${accent};font-size:10px;font-weight:bold;text-decoration:none;">${socialNames[l.label.trim().toLowerCase()] ? image(socialNames[l.label.trim().toLowerCase()],l.label,28) : esc(l.label)}</a></td></tr>${i<links.length-1?`<tr><td align="center" style="padding:8px 0;"><table ${table} width="1"><tr><td height="26" style="width:1px;height:26px;background-color:${accent};font-size:0;line-height:0;">&nbsp;</td></tr></table></td></tr>`:''}`).join('')}</table>` : '';
  ...
  const horizontal = `<tr>${rail?`<td width="28" style="width:28px;padding:0;vertical-align:middle;">${rail}</td>${spacer(35)}`:''}${frame?`<td width="${photoSize+40}" style="width:${photoSize+40}px;padding:0;vertical-align:middle;">${frame}</td>${spacer(42)}`:''}<td style="padding:0;vertical-align:middle;">${details}</td></tr>`;

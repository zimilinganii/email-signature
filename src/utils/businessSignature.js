...
export function businessSignature(p,{preview,accent,background,text,escapeHtml:esc,safeImage}) {
  ...
  const ink=/^#[a-f0-9]{6}$/i.test(p.contactTextColor||'')?p.contactTextColor:(resolveIconColor('auto',strip)==='white'?'#ffffff':'#242424');
  ...
  const location=String(p.businessLocation||'').trim();
  const directions=`https://www.google.com/maps/dir/?api=1&destination=${encodeURIComponent(location)}`;
  const locationHtml=location?`<div data-business-location="address" style="padding-top:8px;max-width:260px;"><table ${table} width="260" style="width:260px;max-width:260px;table-layout:fixed;"><tr><td width="22" style="width:22px;padding:2px 0 0;vertical-align:top;">${image('location',16,background)}</td><td style="padding:0;vertical-align:top;white-space:normal;overflow-wrap:anywhere;word-break:break-word;font-size:13px;line-height:19px;"><a href="${esc(directions)}" target="_blank" rel="noopener noreferrer" title="Get directions in Google Maps" style="color:${text};text-decoration:none;">${esc(location)}</a></td></tr></table></div>`:'';
  const identity=`<td data-business-identity="name" style="padding:0 12px 8px 24px;vertical-align:top;"><div style="font-size:26px;line-height:30px;font-weight:bold;overflow-wrap:anywhere;">${esc(p.fullName)}</div>${p.role?`<div style="font-size:15px;line-height:21px;">${esc(p.role)}</div>`:''}${locationHtml}${p.tagline?`<div style="font-size:11px;padding-top:5px;">${esc(p.tagline)}</div>`:''}</td>`;

...
export function businessSignature(p,{preview,accent,background,text,escapeHtml:esc,safeImage}) {
  ...
  const socialRows=Array.from({length:Math.ceil(socials.length/10)},(_,i)=>{const group=socials.slice(i*10,i*10+10);return `<tr>${socials.length>10&&group.length<10?`<td colspan="${10-group.length}"></td>`:''}${group.join('')}</tr>`;}).join('');
  ...
  const contacts=entries.map(([name,label,url],i)=>`<td style="padding:0 9px;vertical-align:middle;${i?'border-left:2px solid '+ink+';':''}"><table ${table}><tr><td width="21" style="vertical-align:middle;">${image(name,15,strip)}</td><td style="font-size:11px;line-height:15px;white-space:nowrap;word-break:normal;"><a href="${esc(url)}" style="color:${ink};text-decoration:none;white-space:nowrap;">${esc(label)}</a></td></tr></table></td>`).join('');
  return `<table ${table} width="660" data-template="business-logo" style="width:660px;min-width:660px;table-layout:auto;font-family:Arial,Helvetica,sans-serif;background-color:${background};color:${text};border-radius:16px;border-collapse:separate;"><tr><td width="180" rowspan="3" align="center" style="width:180px;padding:20px 0;text-align:center;vertical-align:middle;">${logo}${company?`<div style="padding:8px 12px 0;font-size:22px;line-height:26px;font-weight:bold;overflow-wrap:anywhere;">${esc(company)}</div>`:''}</td><td align="right" height="38" style="height:38px;padding:10px 16px 0 0;"><table ${table} data-business-socials="row">${socialRows}</table></td></tr><tr><td height="94" style="height:94px;padding:0 20px 12px 0;vertical-align:top;"><table ${table} width="100%"><tr><td style="border-left:3px solid #999999;padding:6px 0 22px 24px;"><div style="font-size:26px;line-height:30px;font-weight:bold;overflow-wrap:anywhere;">${esc(p.fullName)}</div>${p.role?`<div style="font-size:15px;line-height:21px;">${esc(p.role)}</div>`:''}${p.tagline?`<div style="font-size:11px;padding-top:5px;">${esc(p.tagline)}</div>`:''}</td></tr></table></td></tr><tr><td height="48" style="height:48px;padding:0 8px 0 22px;background-color:${strip};border-radius:45px 0 14px 0;"><table ${table} width="100%" data-business-contacts="strip" style="width:100%;table-layout:auto;"><tr>${contacts}</tr></table></td></tr></table>`.replace(/style="/g,'style="box-sizing:content-box;');
}

...
export function businessSignature(p,{preview,accent,background,text,escapeHtml:esc,safeImage}) {
  ...
  const locationHtml=location?`<div data-business-location="address" style="padding-top:8px;max-width:260px;white-space:normal;overflow-wrap:anywhere;word-break:break-word;font-size:12px;line-height:18px;color:${text};"><a href="${esc(directions)}" target="_blank" rel="noopener noreferrer" title="Get directions in Google Maps" style="color:${text};text-decoration:none;"><img src="${esc(preview?`${import.meta.env.BASE_URL}icons/${themedIconName('personal-location',p.iconColor==='default'?'auto':p.iconColor,background)}.png`:iconUrl(themedIconName('personal-location',p.iconColor==='default'?'auto':p.iconColor,background)))}" alt="" width="16" height="16" style="vertical-align:middle;border:0;margin-right:5px;" />${esc(location)}</a></div>`:'';

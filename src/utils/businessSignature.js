...
export function businessSignature(p,{preview,accent,background,text,escapeHtml:esc,safeImage}) {
  ...
  const socials=links.map(l=>`<td style="padding:0 0 0 9px;vertical-align:middle;"><a href="${esc(l.url)}" title="${esc(l.label)}">${image(platformIcons[l.label.trim().toLowerCase()]||'website',l.label.trim().toLowerCase()==='linkedin'?36:26,background,l.label)}</a></td>`).join('');

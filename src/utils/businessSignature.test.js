...
describe('logo business layout and social rail',()=>{
 ...
 it('limits both templates to three icons and two thick separators',()=>{
  for(const mode of ['personal','business']){
   ...
   expect(html).not.toContain('title="Facebook"');expect(html.match(/height:20px;background-color/g)).toHaveLength(2);expect(html).toContain('width:3px');expect(html).toContain('personal-instagram.png');
  }
 });
});

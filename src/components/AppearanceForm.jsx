import { resolveIconColor } from '../data/iconPalette';
...
export function AppearanceForm({profile,onChange}) {
  return <><label><span>Signature font</span><select aria-label="Signature font" value={profile.fontFamily||'arial'} onChange={e=>onChange('fontFamily',e.target.value)}>{signatureFonts.map(font=><option key={font.id} value={font.id}>{font.label}</option>)}</select></label><p className="hint">Applies to all signature text. Email apps use a fallback if the font is unavailable.</p><label><span>Signature text size · {profile.fontSize||13}px</span><input aria-label="Signature text size" type="range" min="11" max="17" step="1" value={profile.fontSize||13} onChange={e=>onChange('fontSize',Number(e.target.value))}/></label><p className="hint">Scales all text together, keeping the name larger. Larger text can make the signature wider.</p><div className="color-grid">{[['accent','Accent'],['background','Background'],['textColor','Text'],...(profile.mode==='business'?[['contactStripColor','Contact strip background'],['contactTextColor','Contact text']]:[])].map(([key,label])=><label key={key}><span>{label}</span><input type="color" value={profile[key]||(key==='contactTextColor'?(resolveIconColor('auto',profile.contactStripColor||profile.accent)==='white'?'#ffffff':'#242424'):profile.accent)} onChange={e=>onChange(key,e.target.value)}/></label>)}</div>{profile.mode==='business'&&<button className="text-button" type="button" onClick={()=>onChange('contactTextColor','')}>Use automatic contact-text contrast</button>}<IconColorControl profile={profile} onChange={onChange}/>{profile.mode==='personal'&&<label><span>Signature layout</span><select value={profile.layout} onChange={e=>onChange('layout',e.target.value)}><option value="side">Circular photo · reference layout</option><option value="stack">Stacked</option></select></label>}<p className="hint">{profile.mode==='business'?'Business layout: logo left, name and role beside a divider, platforms top right, and a horizontal contact strip below.':'Personal layout: social links, framed circular portrait, then your name and contact details.'}</p></>;
}

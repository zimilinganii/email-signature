...
export function SocialLinksForm({ links, onChange, errors }) {
  ...
  function addLink() { onChange([...links, { id: crypto.randomUUID(), label: platformOptions.find(name=>!links.some(link=>link.label===name)) || platformOptions[0], url: '', enabled: true }]); }
  return <div className="links-list">{links.map((link, index) => <div className="link-row" key={link.id}>
    <select value={link.label} aria-label="Platform name" onChange={(e) => update(index, 'label', e.target.value)}>{!platformOptions.includes(link.label)&&<option value={link.label}>{link.label}</option>}{platformOptions.map(name=><option key={name} value={name}>{name}</option>)}</select>
  ...
  </div>)}<button type="button" className="secondary" onClick={addLink}>+ Add platform</button><p className="hint">Add as many platforms as you need. Icons resize and wrap into aligned groups; very large signatures may exceed email-provider limits.</p></div>;
}

...
export function SocialLinksForm({ links, onChange, errors }) {
  ...
  function addLink() { if(links.length>=3)return; onChange([...links, { id: crypto.randomUUID(), label: 'Portfolio', url: '', enabled: true }]); }
  ...
  </div>)}<datalist id="platforms">{platformOptions.map((name) => <option key={name} value={name} />)}</datalist><button type="button" className="secondary" disabled={links.length>=3} onClick={addLink}>+ Add platform</button><p className="hint">Choose up to 3 platforms. Icons are separated by a line.</p></div>;
}
